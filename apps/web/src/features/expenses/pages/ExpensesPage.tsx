    ...
    <section className="space-y-4">
      <header className="flex items-start justify-between pt-2">
        <div>
          ...
          <p className="phone-subtitle first-letter:uppercase">{monthLabel}</p>
        </div>
      ...
      <article className="relative overflow-hidden rounded-[22px] bg-[#f57f51] p-5 text-white">
        ...
        <p className="text-sm font-semibold text-white/85">Total gastado</p>
          ...
          <article key={expense.id} className="phone-card flex items-center justify-between p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[#8a9896]">Persona1 · {new Date(expense.expenseDate).toLocaleDateString("es-MX", { day: "2-digit", month: "short" })}</p>
              </div>
            </div>
